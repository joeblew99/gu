...
Views have a unique `ID` generated by default or assignable if desired. This lets views rendering equally well when rendered from the server and allowed to take over on the client. More n this in [Initializing and Server Side Rendering](./docs/InitializationsAndServerSide.md)
